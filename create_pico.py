    ...
    for key in data:
        ...
        if isinstance(value, (str, unicode)):
            ...
            if node_name == "gateway":
                output_dict[node_name].update({"is_gateway":True, "public_ip_address":data["public_ip"]})
...
def create_env_file(input_data):
    """Updates environment file for pnda-cli."""
...
def update_pnda_yaml(mirror_ip, access_key, secret_key, region, branch, os_user):
    """Updates pnda_env.yaml file with user details."""
...
def get_args():
  ...
  - creates pico pnda cluster
    ...
    parser.add_argument('-e', '--cluster-name', help='Cluster name', required=True)
...
def deploy_pnda(args, pnda_path, json_path):
    """Initiates command to deploy PNDA in cli directory"""
...
def get_log_filename(log_path):
    """Returns the log file that got created during PNDA deployment"""
    log_files = []
...
def check_for_status(log_file):
    """Checks for deployment status"""
    with open(log_file, "r") as log:
        for line in log:
            if "Access hints:" in line:
                break
        else:
            return False
    return True
...
def check_log(pnda_path):
    """Returns the status of PNDA deployment"""
    success = False
    if os.path.exists(pnda_path):
        log_path = os.path.join(pnda_path, 'cli/logs')
        log_file = get_log_filename(log_path)
        if log_file is not None:
            success = check_for_status(os.path.join(log_path, log_file))
    return success

def main():
    args = get_args()
    json_file_name = create_production_json_file()
    pnda_path = clone_git_repo("https://github.com/boopalans/pnda-cli.git", args.branch)
    ...
    return_status = check_log(pnda_path)
    if not return_status:
        raise Exception("PNDA deployment failed")
